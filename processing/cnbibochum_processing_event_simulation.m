...
for fId = 1:NumFiles
    ...
    stid = select(bag, 'Topic', '/bci_command');
    ...
    evttid_typ = cellfun(@(m) double(m.Point.Y), msgtid);
    ...
    evttid_typ(evttid_typ == -0.7070) = 26117;
    evttid_typ(evttid_typ == 0.7070)  = 26118;
    % Extracting event time
    cdate = regexp(cfilename, '\.+(\d+.\d+)', 'tokens');
    cdate = cell2mat(cdate{1});
    offset = posixtime(datetime(cdate, 'InputFormat', 'yyyyMMdd.HHmmss'));
    evttid_tim = stid.MessageList.Time + offset;
    evtsys_tim = ssys.MessageList.Time + offset;
